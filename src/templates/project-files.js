...
- **QC reviewers (\`/qc\`)** — ship via the \`claude-guardrails\` plugin, enabled in \`.claude/settings.json\`. On Claude Code **v2.1.195+** an enabled plugin from an external marketplace no longer auto-loads. **In the devcontainer it's installed automatically** (the \`postCreate\` step). If you work outside the devcontainer, accept the install prompt when you trust the folder, or run \`claude plugin install claude-guardrails@claude-scaffold\` once — until then \`/qc\` and the reviewers won't appear.
