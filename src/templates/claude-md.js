...
> **Plugin install (Claude Code v2.1.195+):** enabling the plugin in
> \`.claude/settings.json\` no longer auto-loads it — Claude Code now installs
> plugins from an external marketplace explicitly. **In the devcontainer this is
> automatic** (the \`postCreate\` step runs \`claude plugin install\`). Working
> outside the devcontainer? Accept the install prompt when you trust the folder,
> or run \`claude plugin install claude-guardrails@claude-scaffold\` once. Until
> then \`/qc\` and the reviewers won't appear. Invoke a reviewer by its scoped
> name, e.g. \`--agent claude-guardrails:code-reviewer\`.
